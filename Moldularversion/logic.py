...
#     """
#     folio = None
#     if not text and invoice_type != 'DBA':
#         return "NOT_FOUND"
...
#                 else:
#                      print(f"DEBUG: Folio candidate from DBA filename '{filename}' rejected: {folio_candidate}")

#         # Fallback to Text Search for DBA
...
#     elif not folio:
#         if invoice_type == 'DBA': print(f"DEBUG: Folio ultimately NOT_FOUND for DBA file: {filename}")
#         return "NOT_FOUND"

#     return folio

# def find_references(text):
#     """Extracts Escritura and Acta references from text."""
#     references = []
#     if not text: return []
...
#          potential_singles = [m.group(1).strip() for m in re.finditer(pattern, text, flags) if m.group(1)]
#          for num_str in potential_singles:
#              if num_str.isdigit() and num_str not in found_set:
#                  references.append({"Type": type_name, "Number": num_str})
#                  found_set.add(num_str)
...
#     references = find_references(text) if text else []

#     output_rows = []
#     if text is None and folio != "NOT_FOUND" and folio != "ERROR": # Text failed, but folio found (DBA filename)
#          output_rows.append({
#             "Document Type": "N/A", "Reference Number": "TEXT_EXTRACTION_FAILED",
#             "Invoice Folio": folio, "Source PDF": filename,
#             "Full PDF Path": pdf_path })
#     elif not references: # Text extracted, but no refs found
#          output_rows.append({
...
#     # Handle case where text extraction failed AND folio was not found (DBA)
...
#     # These calls use the passed 'app_instance'
...
#             if results:
#                 for row in results:
#                     if row.get("Document Type") == "ERROR": had_error_in_results = True
#                     all_data.append(row)
#                     rows_added_count += 1
#                 if had_error_in_results: files_with_errors += 1
#                 else: files_processed_count += 1
#             else:
#                  files_with_errors += 1
...
#         print(error_msg)
#         import traceback; traceback.print_exc()
#         master_gui.after(0, app_instance.update_status, f"Error preparing data. See console.")
#         master_gui.after(0, messagebox.showerror, "DataFrame Error", error_msg, parent=master_gui)
#         master_gui.after(0, app_instance.enable_buttons)
#         master_gui.after(10, app_instance.clear_pdf_preview, "Error creating data.")
#         return
...
#         final_message = f"Processing complete. {final_summary}\nData saved to:\n{output_filename}"
#         master_gui.after(0, app_instance.update_status, final_message)
#         master_gui.after(0, app_instance.set_data_and_file, df, output_filename)
#         master_gui.after(10, app_instance.load_data_to_treeview)
...
#         master_gui.after(10, app_instance.set_data_and_file, df, None) # Indicate save failed
#         master_gui.after(20, app_instance.load_data_to_treeview)
#         master_gui.after(30, app_instance.clear_pdf_preview, "Initial save failed (Permission Error). Edits cannot be saved until resolved.")
...
#         master_gui.after(10, app_instance.set_data_and_file, df, None) # Indicate save failed
#         master_gui.after(20, app_instance.load_data_to_treeview)
#         master_gui.after(30, app_instance.clear_pdf_preview, "Initial save failed. Edits cannot be saved.")
