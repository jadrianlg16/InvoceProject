...
class InvoiceProcessorApp:
    def __init__(self, master):
        self.master = master
        master.title("Invoice PDF Processor v2.4 - Interactive Sort") # Version bump
    ...
    # --- Treeview Sorting (NEW IMPLEMENTATION) ---
    def sort_treeview_column(self, col): # Takes only the column ID/Name
        ...
        try:
            ...
            # Coerce to numeric, turning errors (non-numbers) into NaN
            numeric_col_check = pd.to_numeric(sorted_df[col], errors='coerce')
            # Check if the resulting Series has a numeric dtype (int, float)
            is_numeric_type = pd.api.types.is_numeric_dtype(numeric_col_check)
            # Consider it numeric for sorting if *any* values could be coerced AND the result is numeric
            # This handles columns with mixed types or all strings gracefully.
            ...
            if sort_numerically:
                # --- Numeric Sort ---
                # Using a temporary column with the coerced numeric values is safer for mixed types.
                # Non-numeric values (now NaN in numeric_col_check) will be placed according to na_position.
                temp_sort_col = f"__{col}_numeric_sort" # Temporary column name
                sorted_df[temp_sort_col] = numeric_col_check # Assign the coerced Series
                sorted_df = sorted_df.sort_values(by=temp_sort_col, ascending=sort_ascending, na_position='last')
                sorted_df.drop(columns=[temp_sort_col], inplace=True) # Remove the temporary column
            else:
                # --- String Sort ---
                # Sort as string, case-insensitive. Handles numbers-as-strings correctly.
                sorted_df = sorted_df.sort_values(
                    by=col,
                    ascending=sort_ascending,
                    # Robust key function: Convert to string, then lower. Handles None/NaN gracefully.
                    key=lambda x: x.map(lambda s: str(s).lower() if pd.notna(s) else ''),
                    na_position='last' # Place NaN/None values last
                )
            ...
            for c in self.tree["columns"]:
                # Get current header text, remove old arrows
                current_text = self.tree.heading(c, "text").replace(' ▲', '').replace(' ▼', '')
                if c == col:
                    # Add the correct arrow to the sorted column
                    indicator = ' ▲' if sort_ascending else ' ▼'
                    self.tree.heading(c, text=current_text + indicator)
                else:
                    # Ensure other columns have no arrows
                    self.tree.heading(c, text=current_text)

            # --- Reload Treeview with Sorted Data ---
            # This re-populates the tree using the new order in self.dataframe
            # and rebuilds the self._df_index_map based on the new DataFrame index order.
    ...
    def on_tree_double_click(self, event):
        ...
        if region != "cell": return # Click wasn't on a cell content area

        item_id = self.tree.identify_row(event.y) # Get tree item ID ('I001', etc.)
        column_id_str = self.tree.identify_column(event.x) # Get column ID ('#1', '#2', etc.)

        if not item_id or not column_id_str: return # Click was outside valid row/column

        try:
            # Convert column ID string to index (0-based) and get column name
            column_index = int(column_id_str.replace('#', '')) - 1
            if column_index < 0 or column_index >= len(self.tree["columns"]): return # Invalid index
            column_name = self.tree["columns"][column_index]
        except (ValueError, IndexError):
            print(f"Error identifying column from ID: {column_id_str}")
            return

        # Check if the column is designated as editable
        if column_name not in self.editable_columns:
            # Optional: Provide feedback that the column isn't editable
            # self.update_status(f"Column '{column_name}' is not editable.")
            return

        # Get the bounding box of the cell to place the Entry widget
        try:
             bbox = self.tree.bbox(item_id, column=column_id_str)
        except Exception: # Handle potential TclError if item/column removed rapidly
             return
        if not bbox: return # Could be empty if view is scrolled weirdly

        # Get the current value from the Treeview
        current_values = self.tree.item(item_id, 'values')
        if column_index >= len(current_values): return # Should not happen, but safety check
        original_value = current_values[column_index]

        # Store edit state
        self._edit_item_id = item_id
        self._edit_column_id = column_id_str # Store the '#n' ID

        # Create and place the Entry widget
        self._edit_entry = ttk.Entry(self.tree, font=('Segoe UI', 9)) # Match treeview font
        self._edit_entry.place(x=bbox[0], y=bbox[1], width=bbox[2], height=bbox[3])

        # Populate, select text, and focus
        self._edit_entry.insert(0, original_value)
        self._edit_entry.select_range(0, tk.END)
        self._edit_entry.focus_set()

        # Bind events to the Entry widget
        # <Return> (Enter key) and <KP_Enter> (Numpad Enter) save the edit
        ...
        # <FocusOut> (clicking elsewhere) also saves the edit
    ...
    def _save_cell_edit(self, item_id, column_index, column_name):
        ...
        if not self._edit_entry: return # No active edit entry

        # Get the new value BEFORE destroying the widget
        new_value = self._edit_entry.get()

        # Destroy the edit widget and clear state variables *first*
        # This prevents FocusOut from triggering this function again after manual save/cancel
        self._cancel_cell_edit() # Use cancel to handle cleanup

        # --- Update Treeview ---
        # Get current values again (might have changed if events fired rapidly)
        try:
             current_values = list(self.tree.item(item_id, 'values'))
        except tk.TclError:
             print(f"Warning: Treeview item {item_id} no longer exists. Edit not saved.")
             return # Item was deleted while editing

        # Check if the value actually changed
        if column_index < len(current_values) and str(current_values[column_index]) != new_value:
            current_values[column_index] = new_value
            # Update the Treeview item with the new tuple of values
            try:
                 self.tree.item(item_id, values=tuple(current_values))
            except tk.TclError:
                 print(f"Warning: Treeview item {item_id} no longer exists. Edit display not updated.")
                 # Don't proceed to DataFrame update if Treeview update failed

            # --- Update DataFrame ---
            if self.dataframe is not None:
                try:
                    # Get the corresponding DataFrame index from our map
                    df_index = self._df_index_map.get(item_id)

                    if df_index is not None and df_index in self.dataframe.index:
                        # Update the value in the DataFrame at the correct row index and column name
                        # Use .loc for label-based indexing (df_index is the label here)
                        ...
                        # Enable save button if an Excel file path exists
                        if self.current_excel_file:
                            self.save_changes_button.config(state=tk.NORMAL)
                    else:
                        # This indicates a mismatch between Treeview items and the DataFrame map
                    ...
                    # Catch potential errors during DataFrame update (e.g., dtype issues if not handled carefully)
    ...
    def show_context_menu(self, event):
        ...
        # Identify the row under the cursor, if any
        item_id = self.tree.identify_row(event.y)

        # If a row was clicked, ensure it's selected (or part of the selection)
        if item_id:
            # If the clicked item is not already in the selection,
            # clear the previous selection and select only the clicked item.
            if item_id not in self.tree.selection():
                self.tree.selection_set(item_id)
        # If no row was clicked (empty space), the selection remains as is.

        # Determine which menu items should be enabled
        has_selection = bool(self.tree.selection()) # True if any rows are selected
        can_paste = bool(self._clipboard) # True if clipboard has data
        can_add = self.dataframe is not None # Can add if DataFrame exists

        # Configure menu item states
        ...
        try:
            ...
            # Ensure the menu doesn't block other interactions
    ...
    def _add_row(self):
        ...
        if self.dataframe is None:
            ...
            return
        if self._edit_entry: self._cancel_cell_edit() # Cancel any active edit

        try:
            # Create a dictionary representing the new row
            # Initialize with empty strings or appropriate defaults for each column
            new_row_data = {col: "" for col in self.dataframe.columns}
            # Optionally set default values for key columns
            new_row_data["Source PDF"] = "[MANUALLY ADDED]" # Indicate source
            new_row_data["Full PDF Path"] = "" # No associated PDF path
            new_row_data["Invoice Folio"] = ""
            new_row_data["Document Type"] = ""
            new_row_data["Reference Number"] = ""

            # Convert the dictionary to a new DataFrame (with one row)
            new_row_df = pd.DataFrame([new_row_data])

            # Append the new row DataFrame to the existing DataFrame
            # ignore_index=True is crucial: it discards the index of new_row_df (which is 0)
            # and ensures the main DataFrame's index remains unique and sequential after appending.
            ...
            # Store current selection/focus to potentially restore later
            # current_selection = self.tree.selection()
            # current_focus = self.tree.focus()

            self.load_data_to_treeview() # This rebuilds the tree from the updated dataframe

            # --- Optionally scroll to and select the new row ---
            # Get all item IDs after reloading
            children = self.tree.get_children()
            if children:
                new_item_id = children[-1] # The last item added corresponds to the new row
                self.tree.see(new_item_id) # Scroll the view to make the new item visible
                self.tree.selection_set(new_item_id) # Select the new row
                self.tree.focus(new_item_id) # Set focus to the new row

            self.update_status("Added 1 blank row.")
            # Ensure save button is enabled if an Excel path exists
            if self.current_excel_file and not self.dataframe.empty:
                 self.save_changes_button.config(state=tk.NORMAL)
            ...
            import traceback; traceback.print_exc()
            # Attempt to reload treeview even on error to try and resync state
            try:
                self.load_data_to_treeview()
    ...
    def _delete_selected_rows(self):
        ...
        selected_item_ids = self.tree.selection() # Get IDs of selected Treeview items
        ...
        try:
            # --- Map Treeview IDs to DataFrame Indices ---
            indices_to_drop = []
            failed_map_ids = []
            for item_id in selected_item_ids:
                df_index = self._df_index_map.get(item_id) # Get DF index from map
                if df_index is not None and df_index in self.dataframe.index: # Check if index exists in DF
                    indices_to_drop.append(df_index)
                else:
                    # This might happen if the map is out of sync or item was already deleted somehow
            ...
            if failed_map_ids:
                 ...
                 # Optionally, ask user if they want to proceed with deleting the rows that *were* mapped?
                 # For simplicity here, we proceed if *any* rows were mapped successfully.

            if not indices_to_drop:
                 # If no indices could be mapped, stop here.
                 self.update_status("Error: No valid data found for selected rows to delete.")
                 return

            # --- Drop Rows from DataFrame ---
            # Use the collected DataFrame indices to drop rows
            self.dataframe.drop(index=indices_to_drop, inplace=True)

            # --- IMPORTANT: Reset Index ---
            # After dropping rows, the DataFrame index will have gaps.
            # Resetting the index makes it sequential again (0, 1, 2, ...), which is crucial
            # for `load_data_to_treeview` which uses the index as the Treeview item ID.
            # `drop=True` prevents the old index from being added as a new column.
            self.dataframe.reset_index(drop=True, inplace=True)

            # --- Reload Treeview ---
            # Reload the Treeview entirely to reflect the removed rows and the *new* sequential indices.
            self.load_data_to_treeview()
            ...
            if self.current_excel_file and self.dataframe is not None and not self.dataframe.empty:
                 self.save_changes_button.config(state=tk.NORMAL)
            else: # Data might be empty now, or no excel file path exists
                 self.save_changes_button.config(state=tk.DISABLED)
            ...
            import traceback; traceback.print_exc()
            # Attempt reload to resync treeview with potentially partially modified dataframe
            try:
                 self.load_data_to_treeview()
            except Exception as reload_e:
    ...
    def _copy_selected_rows(self):
        ...
        try:
            # --- Map Treeview IDs to DataFrame Indices ---
            # It's important to get the indices corresponding to the *current* view in the Treeview,
            # even if the DataFrame itself isn't currently sorted the same way (though load_data should keep them synced).
            df_indices_to_copy = []
            failed_map_ids = []
            for item_id in selected_item_ids: # Iterate through selected Treeview item IDs
                 df_index = self._df_index_map.get(item_id) # Get corresponding DF index
                 if df_index is not None and df_index in self.dataframe.index:
            ...
            if failed_map_ids:
                 ...
                 # Continue with successfully mapped rows
            ...
            # Use .loc with the list of DataFrame indices to select the desired rows.
            # This preserves the order of selection if needed, although clipboard order doesn't strictly matter here.
            copied_data_df = self.dataframe.loc[df_indices_to_copy]

            # --- Store in Clipboard ---
            # Convert the selected DataFrame rows into a list of dictionaries.
            # 'records' orientation is suitable for this [{'col1': val1, 'col2': val2}, ...]
            # Use deep copy to avoid modifying the original DataFrame if clipboard dicts are changed later (unlikely here, but good practice)
            ...
            if copied_count > 0:
                ...
                # This might happen if mapping failed or the selected rows were empty in the DataFrame somehow
            ...
            self._clipboard = [] # Ensure clipboard is clear on error
            import traceback; traceback.print_exc()
    ...
    def _paste_rows(self):
        ...
        try:
            # --- Prepare Pasted Data ---
            # Create a new DataFrame directly from the clipboard data (list of dictionaries)
            # Pandas handles matching dictionary keys to DataFrame columns automatically.
            ...
            # --- Align Columns ---
            # Ensure the pasted DataFrame only contains columns that exist in the target DataFrame.
            # Reindex `pasted_df` based on the columns of `self.dataframe`.
            # Missing columns in `pasted_df` (that exist in `self.dataframe`) will be filled with `fill_value`.
            # Extra columns in `pasted_df` (not in `self.dataframe`) will be dropped.
            pasted_df = pasted_df.reindex(columns=self.dataframe.columns, fill_value="") # Use "" or None as fill

            if pasted_df.empty:
                 # This could happen if clipboard had only columns not present in the main dataframe
                 self.update_status("No matching columns found between clipboard and table. Nothing pasted.")
                 return

            # --- Append to Main DataFrame ---
            # Use concat similar to _add_row.
            # ignore_index=True is essential to maintain a unique, sequential index.
            self.dataframe = pd.concat([self.dataframe, pasted_df], ignore_index=True)

            # --- Reload Treeview ---
            # Store number of pasted rows to help find them after reload
            num_pasted = len(pasted_df)
            self.load_data_to_treeview() # Reloads the tree with new data and indices

            # --- Optionally Scroll/Select Pasted Rows ---
            children = self.tree.get_children()
            if len(children) >= num_pasted:
                 # Select the block of newly added items
                 first_pasted_item_id = children[-num_pasted]
                 last_pasted_item_id = children[-1]

                 # Clear previous selection and select the new range
                 self.tree.selection_set(children[-num_pasted:])

                 # Scroll to make the first pasted row visible
                 self.tree.see(first_pasted_item_id)
                 # Optionally focus the first pasted row
                 # self.tree.focus(first_pasted_item_id)

            self.update_status(f"Pasted {num_pasted} row(s) from clipboard.")
            # Enable save button if Excel path exists
            if self.current_excel_file and not self.dataframe.empty:
                 self.save_changes_button.config(state=tk.NORMAL)
    ...
    def save_changes_to_excel(self):
        ...
        if self._edit_entry: self._cancel_cell_edit() # Save any pending edit first
        ...
        if not self.current_excel_file:
            # This happens if the initial save failed (e.g., permission error) or was skipped.
            # We could prompt for a *new* file path here.
            ...
            return
        # Check if the *directory* for the file still exists
        ...
        try:
            # Define the columns to include in the Excel file (match initial save)
            excel_columns = ["Source PDF", "Invoice Folio", "Document Type", "Reference Number"]
            ...
            if not cols_to_save:
                 # Should not happen if dataframe is valid, but safety check
                 raise ValueError("No valid columns found in the DataFrame to save.")

            # Create a copy of the relevant columns from the DataFrame
            df_to_save = self.dataframe[cols_to_save].copy()

            # Save to Excel using openpyxl engine (good for .xlsx)
            ...
            # Specific handling for permission errors (file likely open)
            ...
            # Catch other potential errors during saving (disk full, invalid path chars, etc.)
    ...
    def on_treeview_select(self, event):
        ...
        if self._edit_entry and self.tree.focus() != self._edit_entry:
            # Check if we still have the info needed to save
             if self._edit_item_id and self._edit_column_id:
                  try:
                       # Re-derive column index and name
                       col_idx = int(self._edit_column_id.replace('#', '')) - 1
                       col_name = self.tree["columns"][col_idx]
                       self._save_cell_edit(self._edit_item_id, col_idx, col_name)
                  except Exception: # Broad catch if deriving info fails
                       self._cancel_cell_edit() # Fallback to cancel
             else:
                  self._cancel_cell_edit() # Cancel if state is incomplete

        # Get the list of currently selected item IDs
        selected_items = self.tree.selection()

        # If nothing is selected, do nothing (keep current preview or placeholder)
        if not selected_items:
            # Optionally clear preview when selection is cleared:
            # self.clear_pdf_preview("No row selected.")
            # self.current_preview_pdf_path = None
            return

        # Preview the *first* selected item if multiple are selected
        selected_item_id = selected_items[0]

        # Retrieve the full PDF path using the map
        ...
        if pdf_full_path and isinstance(pdf_full_path, str):
            ...
            if os.path.exists(pdf_full_path):
                # Update the preview if the path is different from the current one
                if pdf_full_path != self.current_preview_pdf_path:
                    self.update_pdf_preview(pdf_full_path)
                # Else: Path is the same, no need to reload preview
            else:
                # File path exists in map, but file not found on disk
             ...
             # Row exists in DataFrame map but has no associated PDF path (e.g., manually added row)
             self.clear_pdf_preview("No PDF associated with this row.")
             self.current_preview_pdf_path = None
        elif selected_item_id not in self._df_index_map:
             # Selected item ID isn't even in our map (shouldn't happen with proper loading)
             ...
             # Path data exists but is invalid (e.g., not a string, though unlikely)
    ...
    def clear_pdf_preview(self, message="Select a row to preview PDF"):
        ...
        # Delete existing image from canvas if it exists
        if self._canvas_image_id:
             try:
                 self.pdf_canvas.delete(self._canvas_image_id)
             except tk.TclError: pass # Ignore error if canvas/item already gone
             self._canvas_image_id = None
        # Release PhotoImage reference
        self.pdf_preview_image = None

        # Remove old placeholder widget if it exists
        if self._placeholder_window_id:
            try:
                # Find the item by tag and delete it
                self.pdf_canvas.delete("placeholder")
            except tk.TclError: pass
            self._placeholder_window_id = None

        # Ensure the label widget exists and update its text
        if hasattr(self, 'pdf_placeholder_label') and self.pdf_placeholder_label.winfo_exists():
            ...
            # Recreate label if needed (shouldn't be necessary usually)
            self.pdf_placeholder_label = ttk.Label(self.pdf_canvas, text=message, style="Placeholder.TLabel")

        # Place the placeholder label in the center of the canvas
        # Get current canvas dimensions (provide defaults if not rendered yet)
        ...
        # Create the window item on the canvas containing the label
        self._placeholder_window_id = self.pdf_canvas.create_window(
            canvas_w//2, canvas_h//2,
            window=self.pdf_placeholder_label,
            anchor=tk.CENTER,
            tags="placeholder" # Tag it for easy finding/deletion
        )

        # Reset scroll region to fit the placeholder/canvas size
        # Use after_idle to ensure canvas dimensions are updated first
        self.master.after_idle(self._update_canvas_scrollregion)
        # Clear the currently previewed path state
        self.current_preview_pdf_path = None
    ...
    def update_pdf_preview(self, pdf_path):
        ...
        try:
            ...
            zoom_factor = self.current_zoom_factor # Use the stored zoom level
            mat = fitz.Matrix(zoom_factor, zoom_factor)
            # Get pixmap (rasterized image) of the page with zoom
            pix = page.get_pixmap(matrix=mat, colorspace=fitz.csRGB, alpha=False)
            doc.close() # Close the PDF document

            # Convert pixmap samples to bytes if necessary (depends on fitz version)
            ...
            # Handle various errors during PDF opening or rendering
            ...
            if isinstance(e, fitz.fitz.PasswordError):
                error_msg += "\n(Password Protected?)"
            elif isinstance(e, ValueError): # Catch specific value errors from our checks
                 error_msg += f"\n({e})"
            ...
            # Display user-friendly error on the canvas
    ...
    def zoom_in(self):
        ...
        if self.current_preview_pdf_path: # Only zoom if a PDF is loaded
            new_zoom = min(self.current_zoom_factor * self.zoom_step, self.max_zoom)
            if new_zoom != self.current_zoom_factor: # Avoid reloading if max zoom reached
    ...
    def zoom_out(self):
        ...
        if self.current_preview_pdf_path:
            ...
            if new_zoom != self.current_zoom_factor: # Avoid reloading if min zoom reached
                self.current_zoom_factor = new_zoom
    ...
    def _update_canvas_scrollregion(self):
        """Updates the canvas scroll region to encompass the displayed PDF image or the canvas size."""
        try:
            # If an image is currently displayed
            if self._canvas_image_id and self.pdf_canvas.find_withtag(self._canvas_image_id):
                # Get the bounding box of the image item
                bbox = self.pdf_canvas.bbox(self._canvas_image_id)
                if bbox: # bbox is (x1, y1, x2, y2)
                    # Set scroll region slightly larger than image to avoid clipping issues
                    scroll_bbox = (bbox[0], bbox[1], bbox[2] + 5, bbox[3] + 5)
                    self.pdf_canvas.config(scrollregion=scroll_bbox)
                    return # Done

            # If no image or image not found, set scroll region to current canvas size
            # This ensures scrolling works even when only the placeholder is shown
            current_width = self.pdf_canvas.winfo_width()
            current_height = self.pdf_canvas.winfo_height()
            # Use max(1, ...) to avoid zero dimensions if canvas hasn't rendered fully
            self.pdf_canvas.config(scrollregion=(0, 0, max(1, current_width), max(1, current_height)))

        except tk.TclError as e:
            # Catch errors if canvas/widget destroyed during update
...
if __name__ == "__main__":
    # Check for Pillow before creating the main window (optional but good practice)
    if not PIL_AVAILABLE:
         # Show a simple warning popup if Pillow is missing
         root_check = tk.Tk()
