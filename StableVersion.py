...
# --- REVISED SINGLE REGEX PATTERNS ---
# Added negative lookahead (?!\s+A\s+\d+) to prevent matching start of a range
REGEX_ESCRITURA_SINGLE = r'(?:Escritura|Esc)\s*(?:P[úu]blica)?\s*(?:N[uú]mero|No.?|N°)?\s*[-:\s]?\s*(\d+)\b(?!\s+A\s+\d+)'
# Added negative lookahead (?!\s+(?:A|AL)\s+\d+) to prevent matching start of ranges
REGEX_ACTA_SINGLE = r'Acta\s+Fuera\s+de\s+Protocolo\s+(?:N[uú]mero|No.?|N°)?\s*[-:\s]?\s*(\d+)\b(?!\s+(?:A|AL)\s+\d+)'
# --- END REVISED SINGLE REGEX ---
...
class InvoiceProcessorApp:
    ...
    def sort_treeview_column(self, col, reverse):
        ...
        try:
            ...
            # Determine if the column should be treated as potentially numeric for sorting
            # We use errors='coerce' so non-numeric values become NaN
            numeric_col_check = pd.to_numeric(sorted_df[col], errors='coerce')
            # Consider it numeric if *any* value successfully converted AND the resulting series is numeric
            is_potentially_numeric = not numeric_col_check.isna().all() and pd.api.types.is_numeric_dtype(numeric_col_check)

            print(f"Sorting column '{col}'. Detected as potentially numeric: {is_potentially_numeric}") # Add for debugging

            # --- Use DataFrame.sort_values for BOTH cases ---
            if is_potentially_numeric:
                # Sort numerically. sort_values handles numeric types directly.
                # NaN values will be placed according to na_position.
                sorted_df = sorted_df.sort_values(by=col, ascending=not reverse, na_position='last')
            else:
                # Sort as string, case-insensitive.
                sorted_df = sorted_df.sort_values(
                    by=col,
                    ascending=not reverse,
                    key=lambda x: x.astype(str).str.lower(), # Key for case-insensitivity
                    na_position='last' # Place NaNs (or blanks treated as NaN by key) last
                )

            # Update the main dataframe with the sorted version
            self.dataframe = sorted_df

            # Update Treeview column header indicators
            for c in self.tree["columns"]:
                current_text = self.tree.heading(c, "text").replace(' ▲', '').replace(' ▼', '')
                if c == col:
            ...
            # Reload the treeview with the newly sorted data
            self.load_data_to_treeview()

            # Update the internal sort order state
            self._tree_sort_order[col] = not reverse
