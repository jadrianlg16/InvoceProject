...
**Update**: Prioritizes Folio extraction from filename for DBA invoices based on '_RP_NUMBER_' pattern.
'''
...
# --- NEW: Regex for extracting Folio from DBA filename ---
REGEX_FOLIO_DBA_FILENAME = r'_(\d{1,5})_'
...
def extract_text_from_pdf(pdf_path):
    try:
        ...
        for page_num in range(len(doc)):
            ...
            # Using TEXT_PRESERVE_WHITESPACE can sometimes help regex accuracy
        ...
        # full_text = re.sub(r'[ \t]{2,}', ' ', full_text) # Old method
        # full_text = re.sub(r'\n\s*\n', '\n', full_text) # Old method
        return full_text
...
def find_folio(text, invoice_type, filename=None):
    ...
    For DBA, prioritizes extracting from filename pattern '_RP_NUMBER_'.
    If filename extraction fails for DBA, falls back to text-based regex.
    """
    folio = None
    if not text and invoice_type != 'DBA': # For DBA, we might find it in filename even if text extraction failed
        return "NOT_FOUND" # Or maybe "TEXT_EXTRACTION_FAILED"?

    # --- DBA: Prioritize Filename Extraction ---
    if invoice_type == 'DBA':
        if filename:
            # Attempt extraction using the filename pattern
            filename_match = re.search(REGEX_FOLIO_DBA_FILENAME, filename, re.IGNORECASE)
            if filename_match:
                folio_candidate = filename_match.group(1)
                # Basic validation: Ensure it's digits and not excessively long
                if folio_candidate.isdigit() and len(folio_candidate) < 20:
                    ...
                    # Found in filename, return directly (skip text search for folio)
                    # Final checks (like length > 20) will happen at the end of the function
                else:
                     print(f"DEBUG: Folio candidate from DBA filename '{filename}' rejected (non-digit or too long): {folio_candidate}")

        # --- Fallback to Text Search for DBA if not found in filename OR filename not provided ---
        if not folio: # Only search text if filename search failed or wasn't possible
            print(f"DEBUG: Folio not found in DBA filename '{filename}' (or filename missing). Searching text content.")
            if text: # Check if text is available for searching
                ...
                if match:
                 ...
                 # If text extraction failed AND filename search failed, folio remains None

    # --- Other Invoice Types (Existing Logic) ---
        ...
        if text:
            ...
            # Iterate backwards to find the last "FOLIO:" not preceded by "Folio fiscal"
            ...
            for match in reversed(matches):
                ...
                if not re.search(r'\bFolio\s+fiscal\b', preceding_text, re.IGNORECASE):
                    ...
                    break # Found the likely correct one
            folio = candidate_folio

    # --- Final Checks ---
    if folio and len(folio) > 20:
        return "FOLIO_FISCAL_SUSPECTED" # Or potentially just return the long folio? Modify if needed.
    elif not folio:
        # If it was a DBA and we got here, neither filename nor text search worked
        if invoice_type == 'DBA':
            print(f"DEBUG: Folio ultimately NOT_FOUND for DBA file: {filename}")
        return "NOT_FOUND"

    return folio # Return the found folio (from filename or text)
...
def find_references(text):
    ...
    for match in re.finditer(REGEX_ESCRITURA_LIST_Y, text, flags):
        for num_str_raw in [match.group(1), match.group(2)]:
            ...
            if num_str and num_str.isdigit() and num_str not in found_escritura_numbers: # Basic check
                references.append({"Type": "Escritura", "Number": num_str})
                found_escritura_numbers.add(num_str)
    ...
    for match in re.finditer(REGEX_ACTA_LIST_Y, text, flags):
        for num_str_raw in [match.group(1), match.group(2)]:
            num_str = num_str_raw.strip()
            if num_str and num_str.isdigit() and num_str not in found_acta_numbers: # Basic check
                references.append({"Type": "Acta Fuera de Protocolo", "Number": num_str})
                found_acta_numbers.add(num_str)
    ...
    for num_str in potential_escritura_singles:
        # Check if it wasn't already added by a range/list AND is a digit
        if num_str.isdigit() and num_str not in found_escritura_numbers:
            references.append({"Type": "Escritura", "Number": num_str})
            found_escritura_numbers.add(num_str)
    ...
    for num_str in potential_acta_singles:
        # Check if it wasn't already added by a range/list/special AND is a digit
        if num_str.isdigit() and num_str not in found_acta_numbers:
            references.append({"Type": "Acta Fuera de Protocolo", "Number": num_str})
            found_acta_numbers.add(num_str)
    ...
    def sort_key(item):
        try:
            # Ensure comparison happens as integers
            num_val = int(item["Number"])
        except ValueError:
            num_val = float('inf') # Place non-numeric numbers last within their type
        # Sort primarily by Type, then by the numeric value of the Number
...
def process_single_pdf(pdf_path, invoice_type):
    """ Processes a single PDF file, extracting text, folio, and references. """
    filename = os.path.basename(pdf_path)
    text = extract_text_from_pdf(pdf_path)

    # Handle text extraction failure (pass None text to find_folio for DBA filename check)
    if text is None and invoice_type != 'DBA':
        # For non-DBA, text is required for folio/refs
    ...
    # Find folio (passes filename for DBA check, text for fallback/other types)
    folio = find_folio(text, invoice_type, filename)

    # Find references (only possible if text extraction succeeded)
    references = []
    if text:
        references = find_references(text)
    else:
        # If text extraction failed, we can't find references
        print(f"INFO: Text extraction failed for {filename}, cannot find references.")


    output_rows = []
    # Handle case where text extraction failed but folio might have been found (e.g., DBA filename)
    if text is None and folio != "NOT_FOUND" and folio != "ERROR":
         output_rows.append({
            "Document Type": "N/A", "Reference Number": "TEXT_EXTRACTION_FAILED",
            "Invoice Folio": folio, "Source PDF": filename,
            "Full PDF Path": pdf_path })
    # Handle case where text was extracted, but no references found
    elif not references:
         output_rows.append({
            "Document Type": "N/A", "Reference Number": "N/A",
            "Invoice Folio": folio, "Source PDF": filename,
            "Full PDF Path": pdf_path })
    # Handle case where references were found
    else:
    ...
    # Handle case where text extraction failed AND folio was not found
    if not output_rows and text is None and folio == "NOT_FOUND":
         return [{"Document Type": "ERROR", "Reference Number": "Text Extraction Failed",
                  "Invoice Folio": "NOT_FOUND", "Source PDF": filename,
                  "Full PDF Path": pdf_path}]
...
def run_processing(folder_path, invoice_type, app_instance):
    ...
    files_with_errors = 0 # Counts files where text extraction or critical processing failed
    rows_added_count = 0

    for i, pdf_path in enumerate(pdf_files):
        # Update status periodically
        current_filename = os.path.basename(pdf_path) # Get filename for status message
        ...
        try:
            ...
            # Check for errors explicitly marked by process_single_pdf
            had_error_in_results = False
            if results:
                for row in results:
                    if row.get("Document Type") == "ERROR":
                        had_error_in_results = True
                        print(f"ERROR reported by process_single_pdf for {current_filename}: Folio='{row.get('Invoice Folio')}', Ref='{row.get('Reference Number')}'")
                    all_data.append(row) # Add row even if it's an error row
                    rows_added_count += 1
                if had_error_in_results:
                    files_with_errors += 1
                else:
                    files_processed_count += 1 # Count as processed if no ERROR row returned
            else:
                 # Handle unexpected case where process_single_pdf returns empty list
                 files_with_errors += 1
                 print(f"Warning: No data rows returned by process_single_pdf for {current_filename}")
                 # Add a generic error row for this file to ensure it's represented
                 all_data.append({
                     "Document Type": "ERROR", "Reference Number": "Processing Function Returned Empty",
            ...
            # Catch unexpected runtime errors during process_single_pdf call itself
            files_with_errors += 1
            ...
            app_instance.master.after(0, app_instance.update_status, f"ERROR processing {current_filename}. See console.")
    ...
    # Be more precise: files_processed_count is files *without* errors. total_files is the target.
    final_summary = f"{files_processed_count}/{total_files} files processed without critical errors"
    if files_with_errors > 0:
        final_summary += f" ({files_with_errors} file(s) encountered errors)"
    final_summary += f". {rows_added_count} total rows generated in {processing_time:.2f}s."

    # Handle case where no data was extracted at all (e.g., all files failed critically)
    if not all_data:
        final_message = f"Processing complete. {final_summary}\nNo data extracted (all files may have failed)."
        app_instance.master.after(0, app_instance.update_status, final_message)
        app_instance.master.after(0, app_instance.enable_buttons)
        app_instance.master.after(0, messagebox.showwarning, "Processing Warning", final_message) # Use warning if nothing came out
    ...
    try:
        ...
        # Initial sort: by PDF name, then Doc Type, then Ref Num (handle potential non-numeric refs)
        try:
            # Convert Reference Number to numeric where possible for sorting
            df['Reference Number Num'] = pd.to_numeric(df['Reference Number'], errors='coerce')
            # Sort: Errors/NAs last in Ref Num, then numerically, then alphabetically
    ...
    try:
        ...
        app_instance.master.after(10, app_instance.set_data_and_file, df, None) # Indicate save failed
